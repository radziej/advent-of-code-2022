...
fn main() {
    ...
    let mut steps: Vec<Vec<u32>> = grid
        ...
        .collect();
    steps[destination[0]][destination[1]] = 0;
    let mut visited: HashSet<[usize; 2]> = HashSet::new();
    let mut candidates: HashSet<[usize; 2]> = HashSet::from([destination]);
    ...
    loop {
        ...
        if elevation == 1 {
            println!(
                "Minimum number of steps required to an elevation of 'a' from end: {current_step}"
            );
            break;
        }
