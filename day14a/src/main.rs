...
fn main() {
    ...
    // Vertical threshold, defined as max + 1 (floor at max + 2), constitutes stopping criterion
    let threshold = grid.iter().max_by_key(|p| p[1]).unwrap()[1] + 1;
